...
function newPlayer() {
  return {
    map: 'bedroom', x: 10, y: 6, facing: 'down', ridingBike: false,
    party: [],
    vault: [],
    // Test supply: Hint is reusable, Scanners identify, and Rare Candies speed progression checks.
...
function drawTitle(ctx, ts) {
  ...
  ctx.fillText('팬메이드 GBA 스타일 모험', CX, 52);
  if (Math.floor(t * 1.6) % 2 === 0) {
    ctx.fillStyle = '#fff';
    ctx.fillText('팬 프로젝트 — 공식과 무관함', CX, 154);
  }
  ctx.textAlign = 'left';
}
...
async function titleFlow() {
  ...
  while (true) {
    const opts = [];
    if (SaveSys.hasLocal()) opts.push('이어하기');
    opts.push('새 게임', '저장 코드 불러오기');
    ...
    if (label === '저장 코드 불러오기') {
      ...
      try {
        ...
        UI.toast('저장 데이터를 불러왔습니다!');
        await enterWorld(false);
        return;
      } catch (e) {
        UI.toast('올바르지 않은 저장 코드입니다!');
      }
    }
  }
}
...
async function enterWorld(isNew) {
  ...
  if (isNew) {
    Game.busy = true;
    await UI.sayLines([
      '윌로우브룩의 아침 햇살이 네 방 창문으로 스며든다...',
      '첫 목표: 방 오른쪽 위 계단으로 내려가, 노란 느낌표가 있는 엄마에게 말을 걸자.',
      '엄마에게 준비물을 받은 다음 마을의 메이플 연구소로 가면 된다. 다음 목적지는 ENTER 메뉴의 「임무」에서 언제든 확인할 수 있다.',
      '(방향키/WASD: 이동 · Z/SPACE: 대화/확인 · X: 뒤로 · ENTER: 메뉴 · R: 자전거 · M: 소리)',
    ]);
    Game.busy = false;
    Input.clearPressed();
  }
  UI.toast(`메인 임무: ${Overworld.objective()}`);
}
...
window.addEventListener('load', () => {
  ...
  // Audio playback is user-gesture gated. Unlock on pointer input as well as
  // game-button input so mouse clicks on the title screen start the BGM.
